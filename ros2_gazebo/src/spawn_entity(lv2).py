...
from std_srvs.srv import Empty
	...
	# Gazebo의 spawn_entity 서비스를 호출하여 상호작용
	
	# 1. set arg & run
	# 2. service client ; _spawn_entity -  엔터티 스폰
	# 3. service client ; _delete_entity - 엔터티 삭제 
	# call_async 함수를 통해 비동기 함수 호출
	# rclpy.spin_once 함수를 통해 이벤트 루프를 실행하면서 비동기 호출이 완료될 때까지 대기
	# 정상 작동시 반환값 출력 종료
	# 아니면 반복 후 타임아웃
