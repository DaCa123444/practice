...
//#include "dynamixel.hpp" 			다이나믹셀
//#include "custom_trajectory.hpp" 	커스텀 궤적
//#include "kinematics.hpp" 		로봇의 역기구학 또는 순기구학
...
{
  /*****************************************************************************
    ** Initialize Manipulator Parameter // urdf 같이 조인트 연결부 설명
    *****************************************************************************/
...
//  kinematics_ = new kinematics::SolverUsingCRAndSRPositionOnlyJacobian();
  ...
  if(!sim) //시뮬이 아닌 경우,
  {
    ...
    *****************************************************************************/
    ...
    // 다른 actuator 로 변경 가능 // 제어 루프 시간으로 초기화
    actuator_ = new dynamixel::JointDynamixelProfileControl(control_loop_time);
    
    // Set communication arguments
    STRING dxl_comm_arg[2] = {usb_port, baud_rate};	// usb port, 시리얼 통신 데이터 전송 횟수(/s)
    ...
    STRING joint_dxl_mode_arg = "position_mode";	//위치 제어 모드
  ...
  *****************************************************************************/
  ...
  custom_trajectory_[2] = new custom_trajectory::Rhombus();	//마름모
  custom_trajectory_[3] = new custom_trajectory::Heart();	//하트모양
